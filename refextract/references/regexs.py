...
re_pos_volume = r'(?P<volume_name>\w{1,10})' + \
    re_sep_opt + r'(?P<volume_num>(?:19|20)\d{2})'
# e.g. (LAT2007)
...
RE_OLD_ARXIV = [compute_arxiv_re(*i) for i in six.iteritems(old_arxiv)]
...
re_kb_line = re.compile(r'^\s*(?P<seek>[^\s].*)\s*---\s*(?P<repl>[^\s].*)\s*$',
                        re.UNICODE)
...
re_page = r"(%s|%s)" % (re_jinst_page, re_page_prefix +
...
def get_reference_line_numeration_marker_patterns(prefix=u''):
    ...
    patterns = [
        ...
        # [<letters and numbers]
        space + title + g_name +
        r'\[\s*[a-zA-Z:-]+\+?\s?(\d{1,4}[A-Za-z:-]?)?\s*\]' + g_close,
        # {1}
        space + title + g_name + r'\{\s*(?P<marknum>\d+)\s*\}' + g_close,
        # (1)
        space + title + g_name + r'\<\s*(?P<marknum>\d+)\s*\>' + g_close,
