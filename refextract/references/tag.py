...
def tag_reference_line(line, kbs, record_titles_count):
    ...
    working_line2 = re_punctuation.sub(" ", working_line2)

    # Remove multiple spaces from the line, recording
    # information about their coordinates:
    removed_spaces, working_line2 = remove_and_record_multiple_spaces_in_line(
        working_line2
    )
    ...
    journals_matches_more, working_line2, line_titles_count = identify_journals(
        working_line2, kbs["journals"]
    )
    ...
    if working_line2.upper().find("IBID") != -1:
...
def process_reference_line(
    ...
    if len(journals_matches) + len(pprint_repnum_len) + len(publishers_matches) == 0:
        ...
        replacement_types = get_replacement_types(
            journals_keys, reports_keys, publishers_keys
        )
        replacement_locations = sorted(replacement_types.keys())

        tagged_line = ""  # This is to be the new 'working-line'. It will
        ...
        for replacement_index in replacement_locations:
            ...
            true_replacement_index, extras = account_for_stripped_whitespace(
                spaces_keys,
                removed_spaces,
                replacement_types,
                pprint_repnum_len,
            ...
            if replacement_types[replacement_index] == "journal":
                # Add a tagged periodical TITLE into the line:
                rebuilt_chunk, startpos, previous_match = add_tagged_journal(
            ...
            elif replacement_types[replacement_index] == "reportnumber":
                # Add a tagged institutional preprint REPORT-NUMBER
                # into the line:
                rebuilt_chunk, startpos = add_tagged_report_number(
                    reading_line=working_line,
            ...
            elif replacement_types[replacement_index] == "publisher":
                rebuilt_chunk, startpos = add_tagged_publisher(
                    reading_line=working_line,
    ...
    * arXiv:2007.12.1111v2
    ...
    def tagger(match):
        ...
        if match.group("suffix"):
            ...
            groups["suffix"] = ""
        return (
            "<cds.ARXIV>arXiv:%(year)s%(month)s.%(num)s%(suffix)s</cds.ARXIV>" % groups
        )
...
def tag_arxiv_more(line):
    ...
    * arXiv:1022111 [hep-ph] which transforms to hep-ph/1022111
    ...
    for report_re, report_repl in RE_OLD_ARXIV:
        ...
        line = report_re.sub("<cds.ARXIV>" + report_number + "</cds.ARXIV>", line)
    return line
...
def add_tagged_report_number(
    reading_line, len_reportnum, reportnum, startpos, true_replacement_index, extras
):
    ...
    """
    rebuilt_line = ""  # The segment of the line that's being rebuilt to
    ...
    rebuilt_line += "<cds.REPORTNUMBER>%(reportnum)s</cds.REPORTNUMBER>" % {
        "reportnum": reportnum
    }
...
def extract_series_from_volume(volume):
    patterns = (re_series_from_numeration, re_series_from_numeration_after_volume)
    for p in patterns:
...
def create_numeration_tag(info):
    ...
    numeration_tags = " <cds.VOL>%s</cds.VOL>" % series_and_volume
    if info.get("year", False):
        numeration_tags += " <cds.YR>(%(year)s)</cds.YR>" % info
    if info.get("page_end", False):
        numeration_tags += " <cds.PG>%(page)s-%(page_end)s</cds.PG>" % info
    else:
        numeration_tags += " <cds.PG>%(page)s</cds.PG>" % info
    return numeration_tags


def add_tagged_journal(
    reading_line,
    journal_info,
    previous_match,
    startpos,
    ...
    if journal_info.upper().find("IBID") != -1:
        ...
        if ";" in standardised_titles[journal_info]:
            title, series = standardised_titles[journal_info].rsplit(";", 1)
            series = series.strip()
            previous_match = {"title": title, "series": series}
        else:
            title = standardised_titles[journal_info]
            previous_match = {"title": title, "series": None}
...
def account_for_stripped_whitespace(
    spaces_keys,
    removed_spaces,
    replacement_types,
    len_reportnums,
    ...
    for space in spaces_keys:
        if space < true_replacement_index:
            ...
            and replacement_types[replacement_index] == "journal"
            and space
            < (spare_replacement_index + len(journals_matches[replacement_index]))
        ):
            ...
            space >= spare_replacement_index
            and replacement_types[replacement_index] == "reportnumber"
...
def strip_tags(line):
    ...
    # we can do this
    re_tag = re.compile(r"<cds\.[A-Z]+>[^<]*</cds\.[A-Z]+>|<cds\.[A-Z]+ />", re.UNICODE)
...
def identify_and_tag_authors(line, authors_kb):
    ...
    if matched_authors:
                ...
                matched_positions.append(
                    {
                        ...
                        "text_before": preceeding_text_string[
                            preceeding_text_start : match.start()
                        ],
                        "auth_no": auth_no,
        ...
        for m in matched_positions:
            ...
            if (
                not dump_in_misc
                and not (m["multi_auth"] or m["multi_surs"])
                ...
                if weaker_match and not (
                    weaker_match.group("es") or weaker_match.group("ee")
                ):
            ...
            if (
                len(output_line) > m["end"]
                and output_line[m["end"]].strip(" ,.") == ";"
            ):
                add_to_misc = ";"

            # Standardize eds. notation
            tmp_output_line = re.sub(
                re_ed_notation, "(ed.)", output_line[start:end], flags=re.IGNORECASE
            )
            # Standardize et al. notation
            tmp_output_line = re.sub(
                re_etal, "et al.", tmp_output_line, flags=re.IGNORECASE
            )
            # Strip
            ...
            # editors
                ...
                # Standardize et al. notation
                tmp_output_line = re.sub(
                    re_etal, "et al.", m["author_names"], flags=re.IGNORECASE
                )
...
def identify_ibids(line):
    ...
    for m_ibid in re_ibid.finditer(line):
        ...
        line = (
            line[0 : m_ibid.start()] + "_" * len(m_ibid.group(0)) + line[m_ibid.end() :]
        )
...
def identify_report_numbers(line, kb_reports):
    ...
    repnum_categs = sorted(
        repnum_standardised_categs.keys(), key=lambda x: (len(x[1]), x), reverse=True
    )
    ...
    for categ in repnum_categs:
        ...
        for repnum_match in repnum_matches_iter:
            ...
            line = (
                line[0 : repnum_match.start(1)]
                + "_" * len(repnum_match.group(1))
                + line[repnum_match.end(1) :]
            )
            ...
            repnum_matches_matchlen[repnum_match.start(1)] = len(repnum_match.group(1))
            # standardised replacement for the matched preprint report number:
            repnum_matches_repl_str[repnum_match.start(1)] = (
                repnum_standardised_categs[categ] + numeration_match
            )
...
def identify_and_tag_URLs(line):
    ...
    for m_tagged_url in m_tagged_url_iter:
        ...
        line = line[0:startposn] + "_" * matchlen + line[endposn:]
    ...
    for url_position in found_url_positions:
        identified_urls.append(
            (found_url_urlstring[url_position], found_url_urldescr[url_position])
        )
