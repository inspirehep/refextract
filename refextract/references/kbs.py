...
def create_institute_numeration_group_regexp_pattern(patterns):
    ...
    grouped_numeration_pattern = "(?P<numn>%s)" % "|".join(patterns_list)
...
def build_reportnum_kb(fpath):
    ...
    def _add_institute_preprint_patterns(
        ...
        if preprint_classifications and preprint_numeration_ptns:
            ...
            ordered_patterns = order_reportnum_patterns_bylen(preprint_numeration_ptns)
            # create a grouped regexp for numeration part of
            # preprint reference:
            numeration_regexp = create_institute_numeration_group_regexp_pattern(
                ordered_patterns
            )
    ...
    re_preprint_classification = re.compile(
    ...
    with file_resolving(fpath) as fh:
        for rawline in fh:
            ...
            m_preprint_classification = re_preprint_classification.search(rawline)
...
def build_journals_kb(knowledgebase):
    ...
    for seek_phrase, repl in knowledgebase:
        ...
        seek_ptn = re.compile(r"(?<!\w)(%s)\W" % re.escape(seek_phrase), re.UNICODE)
    ...
    for repl_term in repl_terms:
        ...
        raw_repl_phrase = re_punctuation.sub(" ", raw_repl_phrase)
        raw_repl_phrase = re_group_captured_multiple_space.sub(" ", raw_repl_phrase)
